...
const OAKLAND_BAY_BRIDGE_POLYGON: atlas.data.Feature<atlas.data.Polygon, object> = new atlas.data.Feature(
...
const AzureMapView: React.FC = () => {
  ...
  // Add supply chain data to the map
  useEffect(() => {
    if (!supplyChain || !dataSourceRef.current) return;
    ...
    supplyChain.nodes.forEach(node => {
      ds.add(new atlas.data.Feature(
        ...
        { name: node.name, type: node.type, inventory: node.inventory }
      ));
    });
    // Add routes as lines
    ...
    // Add bridge polygon
    ds.add(OAKLAND_BAY_BRIDGE_POLYGON);
  }, [supplyChain]);
